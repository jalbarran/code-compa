...
- [x] Set up the monorepo with the initial structure.
- [x] Write the first spec (`specs/spec-01-go-bridge/`) covering the sidecar lifecycle, port allocation, and IPC contract.
- [x] Develop a basic VS Code extension in TypeScript that runs the Go background process safely.
- [x] Write Go code to detect the local Wi-Fi IP and start the Connect-RPC HTTP server.
- [x] Implement the logic to inject `chmod +x` permissions from Node.js on Mac/Linux.
- [x] Define `.proto` files under `proto/codecompa/v1/` and generate Go + TypeScript stubs with `make proto`.

### 📱 Phase 2: Mobile Companion (Expo)

- [x] Write the spec (`specs/spec-02-mobile-companion/`) covering QR handshake, streaming connection, and HITL UI flow.
- [x] Initialize the mobile app using Expo Router.
- [x] Integrate `expo-camera` to read the QR code generated by the IDE.
- [x] Validate real-time Server-Streaming Connect-RPC connection (PC ↔ Phone).
- [x] Build type-safe Connect-RPC client from generated TypeScript stubs.

### 🚀 Phase 3: Ecosystem and Integration

- [x] Connect real event hooks from the IDE agents to the Go server.
- [x] Design mobile UI screens tailored for summarizing code diffs and prompts.
